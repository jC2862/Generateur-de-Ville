...
class DrawableArea :
    ...
    def helper(self, top, left, right) :
    ...
    def create_inside_rect(self, top, left, right) : 
        epsilon = self.epsilon
        ...
        if 1 - epsilon < self.face.normal[0] <= 1 or -1 <= self.face.normal[1] < -1 + epsilon :
            ...
            if -1 <= self.face.normal[1] < - 1 + epsilon :
                ...
                top, left, right = self.helper(top, left, right)
                 
                new_bl = (bottom_left[0] + left, bottom_left[1], bottom_left[2])  
            ...
            # le y et le z changent    
                ...
                new_bl = (bottom_left[0], bottom_left[1] + left, bottom_left[2])  
        ...
        #on evalue une face // a l axe x     
            ...
            if 1 - epsilon < self.face.normal[1] <= 1 :
                self.width = top_right[0] - bottom_left[0]
                self.height = top_right[2] - bottom_left[2]
                top, left, right = self.helper(top, right, left)
    ...
    def __init__(self, obj_name, face, top, left, right) :
        self.epsilon = 0.0002
        ...
        self.mesh = bm
        self.face = face
        self.rect = self.create_inside_rect(top, left, right)
